...
class MyFirstWidget(BoxLayout):
	...
	def check_status(self, btn):
		...
		print('text input text is: {txt}'.format(txt=self.text_inpt))
...
```
...
At this point self.txt_inpt is *None*. In Kv lang this property is updated to hold the instance of the `TextInput` referenced by the if *txt_inpt*.:
...
When yout kv file is parsed, kivy collects all the widgets tagged with id's and places them in this *self.ids* dictionary type property. That means you can also iterate over these widgets and access them dictionary style:
